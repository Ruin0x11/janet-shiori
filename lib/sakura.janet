...
(defn flatten-macro-result
  "Flattens a list with the :recurse sentinel at the beginning. This
  is needed to splice the result of a macro computation into the final
  array, because only one value can be returned from a macro result."
  [i]
  (def arr @[])
  (each x i
...
(defmacro sakura-recurse [i tag1 tag2 idx]
  ~(let [it (tuple/slice ,i ,idx)]
     ...
     # if it is the return value of a function/macro.
     [recurse-sentinel ,tag1 (flatten-macro-result (map sakura-reify it)) ,tag2]))
...
(put meths :quick-section quick-section)
